...
async function openPhotoViewer(d){
  try{
    ...
    let pinchStartDist=0,pinchStartScale=1,pinchStartX=0,pinchStartY=0,pinchStartMidX=0,pinchStartMidY=0;
    let dragPointer=null,dragLastX=0,dragLastY=0;

    const clamp=()=>{
      const sw=stage.clientWidth,sh=stage.clientHeight;
      const iw=img.getBoundingClientRect().width/Math.max(scale,0.0001);
      const ih=img.getBoundingClientRect().height/Math.max(scale,0.0001);
      const maxX=Math.max(0,(iw*scale-sw)/2);
      const maxY=Math.max(0,(ih*scale-sh)/2);
      x=Math.max(-maxX,Math.min(maxX,x));
      y=Math.max(-maxY,Math.min(maxY,y));
    };
    const apply=()=>{
      scale=Math.max(1,Math.min(5,scale));
      if(scale===1){x=0;y=0}else clamp();
      img.style.transform=`translate3d(${x}px,${y}px,0) scale(${scale})`;
    };
    const zoomAt=(factor,cx=stage.clientWidth/2,cy=stage.clientHeight/2)=>{
      const old=scale,next=Math.max(1,Math.min(5,scale*factor));
      if(next===old)return;
    ...
    const resetViewer=()=>{scale=1;x=0;y=0;apply()};

    const finishLoad=()=>{resetViewer();};
    if(img.complete) finishLoad(); else img.addEventListener('load',finishLoad,{once:true});
    ...
    stage.addEventListener('pointerdown',e=>{
      ...
      if(pointers.size===1){
        dragPointer=e.pointerId;dragLastX=e.clientX;dragLastY=e.clientY;
        const now=performance.now();
        const r=stage.getBoundingClientRect();
        if(now-lastTap<280){
          zoomAt(scale>1?0.5:2,e.clientX-r.left,e.clientY-r.top);
        }
        lastTap=now;
        ...
        pinchStartScale=scale;pinchStartX=x;pinchStartY=y;
        const m=midpoint(pts[0],pts[1]),r=stage.getBoundingClientRect();
        pinchStartMidX=m.x-r.left;pinchStartMidY=m.y-r.top;
      }
    },{passive:false});
    ...
    stage.addEventListener('pointermove',e=>{
      ...
      if(pointers.size===2){
        ...
        if(pinchStartDist>0){
          ...
          const m=midpoint(pts[0],pts[1]),r=stage.getBoundingClientRect();
          const mx=m.x-r.left,my=m.y-r.top,ratio=scale/pinchStartScale;
          x=(pinchStartX-pinchStartMidX)*ratio+(mx-pinchStartMidX);
          y=(pinchStartY-pinchStartMidY)*ratio+(my-pinchStartMidY);
          apply();
        }
        return;
      }
      if(dragPointer===e.pointerId&&scale>1){
        x+=e.clientX-dragLastX;y+=e.clientY-dragLastY;
        dragLastX=e.clientX;dragLastY=e.clientY;apply();
      }
    },{passive:false});
    ...
    stage.addEventListener('wheel',e=>{
      e.preventDefault();
      const r=stage.getBoundingClientRect();
      zoomAt(e.deltaY<0?1.15:.87,e.clientX-r.left,e.clientY-r.top);
    },{passive:false});
