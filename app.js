...
const AZELAIC_DAYS=[3,6,10,13,17,20,24,27,30],STORAGE_KEY='skinRoutineState',PHOTO_DB='skinRoutinePhotosV4',PHOTO_STORE='photos',PHOTO_DAYS=[1,7,14,20,25,30];
...
function planDate(d){const x=dObj(state.startDate);x.setDate(x.getDate()+d-1);return x.toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'})}
...
function today(){const d=previewDay||currentDay(),r=routine(d);return `<div class="home" id="home"><section class="routine-stack"><div class="routine-card morning-card"><div class="routine-head"><span class="routine-icon">☀️</span><div><b>Morning</b></div></div><div class="task-grid morning-grid">${r.morning.map(x=>task(d,...x)).join('')}</div></div><div class="routine-card night-card"><div class="routine-head"><span class="routine-icon">🌙</span><div><b>Night</b></div></div><div class="task-grid night-grid">${r.night.map(x=>task(d,...x)).join('')}</div></div></section></div>`}
function days(){const cd=currentDay();return `<div class="tab"><header><div><div class="eyebrow">YOUR PLAN</div><h2>30 Days</h2><p>Tap a day to preview.</p></div><span class="chip">${completedDays()}/30</span></header><div class="days-grid">${Array.from({length:30},(_,i)=>{const n=i+1,p=pct(n),photoDue=PHOTO_DAYS.includes(n)&&!state.photoDays[n];return `<button type="button" class="day-card ${p===100?'done':''} ${n===cd?'today':''}" data-fill="${p}" data-action="preview" data-preview="${n}"><span class="day-fill"></span><strong>${n}</strong><small>${p}%</small>${photoDue?'<i class="photo-dot" aria-label="Progress photo due"></i>':''}</button>`}).join('')}</div><div class="tab-note">Progress photo reminders: Day 1 · 7 · 14 · 20 · 25 · 30.</div></div>`}
...
function progress(){const cd=completedDays(),p=Math.round(cd/30*100);return `<div class="tab progress-tab"><header><div><div class="eyebrow">YOUR PROGRESS</div><h2>Progress</h2><p>Keep your routine consistent.</p></div><span class="chip">${p}%</span></header><section class="progress-card"><div class="big-number"><strong>${cd}</strong><small>/30 days</small></div><div><b>Completed days</b><small>🔥 ${streak()} day streak</small></div><div class="progress-line wide"><i style="width:${p}%"></i></div></section><div class="photo-title"><b>📸 Progress photos</b><small>Day 1 · 7 · 14 · 20 · 25 · 30</small></div><div class="photos">${PHOTO_DAYS.map(photoCard).join('')}</div><div class="tab-note">Use similar lighting, distance and angle for a fair visual comparison.</div></div>`}
function render(tab){if(tab)currentTab=tab;document.querySelectorAll('.nav-item').forEach(b=>b.classList.toggle('active',b.dataset.tab===currentTab));const s=document.getElementById('screen');s.innerHTML=currentTab==='today'?today():currentTab==='days'?days():progress();updateTopInfo();applyTheme();if(currentTab==='days')document.querySelectorAll('.day-card').forEach(el=>{const f=Number(el.dataset.fill||0);el.style.setProperty('--fill',f+'%');el.classList.toggle('zero-fill',f===0)});if(currentTab==='progress')setTimeout(hydratePhotos,0)}
function updateTopInfo(){const d=previewDay||currentDay(),p=pct(d),day=document.getElementById('topDay'),date=document.getElementById('topDate'),bar=document.getElementById('topInfo');if(day)day.textContent=d;if(date)date.textContent=planDate(d);if(bar){bar.style.setProperty('--top-fill',p+'%');bar.classList.toggle('zero-fill',p===0)}}
...
async function openPhotoViewer(d){
  try{
    const b=await getPhoto(d); if(!b)return;
    const u=URL.createObjectURL(b),overlay=document.createElement('div');
    overlay.className='photo-viewer';
    overlay.innerHTML=`<button class="photo-viewer-close" aria-label="Close">×</button><button class="photo-zoom-out" aria-label="Zoom out">−</button><button class="photo-zoom-in" aria-label="Zoom in">+</button><div class="photo-viewer-stage"><img src="${u}" alt="Day ${d} progress photo" draggable="false"></div><div class="photo-viewer-title">Day ${d} · pinch / drag to zoom and view</div>`;
    document.body.appendChild(overlay);
    requestAnimationFrame(()=>overlay.classList.add('popup-ready'));
    const stage=overlay.querySelector('.photo-viewer-stage'),img=overlay.querySelector('img');
    let scale=1,x=0,y=0,lastDist=0,lastX=0,lastY=0,panning=false,pointerId=null,lastTap=0;
    const clamp=()=>{
      const sw=stage.clientWidth,sh=stage.clientHeight;
      const iw=img.offsetWidth||sw,ih=img.offsetHeight||sh;
    ...
    const apply=()=>{scale=Math.max(1,Math.min(5,scale));if(scale===1){x=0;y=0}else clamp();img.style.transform=`translate3d(${x}px,${y}px,0) scale(${scale})`};
    const zoomAt=(factor,cx=stage.clientWidth/2,cy=stage.clientHeight/2)=>{const old=scale;const next=Math.max(1,Math.min(5,scale*factor));if(next===old)return;const ratio=next/old;x=(x-cx)*ratio+cx;y=(y-cy)*ratio+cy;scale=next;apply()};
    overlay.querySelector('.photo-zoom-in').onclick=e=>{e.stopPropagation();zoomAt(1.25)};
    overlay.querySelector('.photo-zoom-out').onclick=e=>{e.stopPropagation();zoomAt(.8)};
    stage.addEventListener('pointerdown',e=>{
      if(e.pointerType==='mouse'&&e.button!==0)return;
      stage.setPointerCapture?.(e.pointerId);pointerId=e.pointerId;
      if(scale>1){panning=true;lastX=e.clientX;lastY=e.clientY}
      const now=performance.now();if(now-lastTap<280){zoomAt(scale>1?0.5:2,e.clientX-stage.getBoundingClientRect().left,e.clientY-stage.getBoundingClientRect().top)}lastTap=now;
    });
    stage.addEventListener('pointermove',e=>{if(!panning||e.pointerId!==pointerId||scale<=1)return;e.preventDefault();x+=e.clientX-lastX;y+=e.clientY-lastY;lastX=e.clientX;lastY=e.clientY;apply()});
    const endPan=()=>{panning=false;pointerId=null};
    ...
    stage.addEventListener('touchstart',e=>{if(e.touches.length===2){lastDist=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY);panning=false}},{passive:true});
    stage.addEventListener('touchmove',e=>{if(e.touches.length===2){e.preventDefault();const dist=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY);if(lastDist){scale=Math.max(1,Math.min(5,scale+(dist-lastDist)/260));apply()}lastDist=dist}},{passive:false});
    stage.addEventListener('touchend',()=>{lastDist=0},{passive:true});
    stage.addEventListener('wheel',e=>{e.preventDefault();const r=stage.getBoundingClientRect();zoomAt(e.deltaY<0?1.15:.87,e.clientX-r.left,e.clientY-r.top)},{passive:false});
    const close=()=>{URL.revokeObjectURL(u);overlay.remove()};
    overlay.addEventListener('click',e=>{if(e.target===overlay||e.target.closest('.photo-viewer-close'))close()});
  }catch(e){alert('Unable to open this photo.')}
}
...
screen.addEventListener('click',e=>{try{
  ...
  if(action==='photo-view'){
    ...
    if(!state.photoDays[d])choosePhoto(d); else openPhotoViewer(d);
    return;
  }
...
let copyTimer=null,copyTarget=null;
function copyProductNameStart(e){const el=e.target.closest('.product-name');if(!el||currentTab!=='today')return;copyTarget=el;clearTimeout(copyTimer);copyTimer=setTimeout(async()=>{if(copyTarget!==el)return;const text=el.textContent.trim();try{if(navigator.clipboard?.writeText)await navigator.clipboard.writeText(text);else{const ta=document.createElement('textarea');ta.value=text;ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.select();document.execCommand('copy');ta.remove()}el.classList.add('copied');setTimeout(()=>el.classList.remove('copied'),420)}catch(err){}},520)}
function copyProductNameEnd(e){const el=e.target.closest('.product-name');if(el&&el===copyTarget){clearTimeout(copyTimer);copyTimer=null;copyTarget=null}}
screen.addEventListener('pointerdown',copyProductNameStart,{passive:true});screen.addEventListener('pointerup',copyProductNameEnd,{passive:true});screen.addEventListener('pointercancel',copyProductNameEnd,{passive:true});screen.addEventListener('contextmenu',e=>{if(e.target.closest('.product-name')||e.target.closest('.task-img')||e.target.closest('.photo-box')||e.target.closest('.photo-viewer-stage')||e.target.closest('.product-viewer-stage'))e.preventDefault()});
screen.addEventListener('dragstart',e=>{if(e.target.closest('.task-img')||e.target.closest('.photo-box')||e.target.closest('.photo-viewer-stage')||e.target.closest('.product-viewer-stage'))e.preventDefault()});
...
function moveDay(delta){
  const d=previewDay||currentDay(),next=Math.max(1,Math.min(30,d+delta));if(next===d)return;
  const direction=delta>0?'left':'right',old=document.getElementById('home');
  if(!old){previewDay=next===currentDay()?null:next;render('today');return}
  const oldRect=old.getBoundingClientRect(),screenEl=document.getElementById('screen');
  const oldClone=old.cloneNode(true);oldClone.removeAttribute('id');oldClone.className='home recents-old';
  Object.assign(oldClone.style,{position:'fixed',left:oldRect.left+'px',top:oldRect.top+'px',width:oldRect.width+'px',height:oldRect.height+'px',margin:'0',zIndex:'30',pointerEvents:'none',overflow:'hidden'});
  screenEl.appendChild(oldClone);
  previewDay=next===currentDay()?null:next;render('today');
  const fresh=document.getElementById('home');if(!fresh){oldClone.remove();return}
  const freshClone=fresh.cloneNode(true);freshClone.removeAttribute('id');freshClone.className='home recents-new';
  Object.assign(freshClone.style,{position:'fixed',left:oldRect.left+'px',top:oldRect.top+'px',width:oldRect.width+'px',height:oldRect.height+'px',margin:'0',zIndex:'29',pointerEvents:'none',overflow:'hidden'});
  screenEl.appendChild(freshClone);
  fresh.style.visibility='hidden';
  oldClone.classList.add(direction==='left'?'recent-out-left':'recent-out-right');
  freshClone.classList.add(direction==='left'?'recent-in-left':'recent-in-right');
  setTimeout(()=>{oldClone.remove();freshClone.remove();fresh.style.visibility='visible'},230);
}
...
if('serviceWorker'in navigator)window.addEventListener('load',()=>{navigator.serviceWorker.register('./service-worker.js?v=18',{scope:'./'}).then(r=>r.update()).catch(()=>{})});
