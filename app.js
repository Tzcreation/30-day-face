...
screen.addEventListener('click',e=>{try{
  ...
  if(action==='photo-view'){
    ...
    if(state.photoDays[d])openPhotoViewer(d); else choosePhoto(d);
    return;
  }
...
screen.addEventListener('pointerdown',copyProductNameStart,{passive:true});screen.addEventListener('pointerup',copyProductNameEnd,{passive:true});screen.addEventListener('pointercancel',copyProductNameEnd,{passive:true});screen.addEventListener('contextmenu',e=>{if(e.target.closest('.product-name, .task-img, .photo-box'))e.preventDefault()});
screen.addEventListener('selectstart',e=>{if(e.target.closest('.task-img, .photo-box'))e.preventDefault()});
...
function showSwipeIndicator(next,delta){
  const old=document.querySelector('.swipe-indicator'); if(old)old.remove();
  const el=document.createElement('div'); el.className='swipe-indicator';
  el.innerHTML=`<span>${delta>0?'→':'←'}</span><b>Day ${next}</b>`;
  document.body.appendChild(el);
  requestAnimationFrame(()=>el.classList.add('show'));
  setTimeout(()=>{el.classList.remove('show');setTimeout(()=>el.remove(),120)},260);
}
function moveDay(delta){
  ...
  showSwipeIndicator(next,delta);
  previewDay=next===currentDay()?null:next;render('today');
  const fresh=document.getElementById('home'); if(!fresh)return;
  const freshRect=fresh.getBoundingClientRect();
  fresh.style.willChange='transform';
  fresh.style.transform=`translate3d(${direction==='left'?'8':'-8'}px,0,0)`;
  fresh.style.transition='transform .18s cubic-bezier(.22,.8,.2,1)';
  const makeHalf=(side)=>{
  ...
  requestAnimationFrame(()=>{
    fresh.style.transform='translate3d(0,0,0)';
    left.classList.add(direction==='left'?'split-left-out':'split-left-in');
    right.classList.add(direction==='left'?'split-right-out':'split-right-in');
  });
  setTimeout(()=>{left.remove();right.remove();fresh.style.willChange='auto';fresh.style.transition='';fresh.style.transform=''},190);
}
...
if('serviceWorker'in navigator)window.addEventListener('load',()=>{navigator.serviceWorker.register('./service-worker.js?v=19',{scope:'./'}).then(r=>r.update()).catch(()=>{})});
